...
function loadState(state) {
  ...
  els.target.value = state.target || "github";
...
function updatePreview() {
  ...
  if (els.breaking.checked) parts.push("", "BREAKING CHANGE: " + (body || "描述破坏性变更"));
...
function renderStatus(status) {
  ...
  const cells = [
    ["分支", branch],
    ["GitHub", status.github_url || "未配置"],
    ["Gitee", status.gitee_url || "未配置"],
    ["领先/落后", `${ahead} / ${behind}`],
  ...
  els.statusGrid.innerHTML = cells
    .map(([k, v]) => `<div class="stat"><span class="stat-k">${k}</span><span class="stat-v">${esc(v)}</span></div>`)
    .join("");
  ...
  els.changes.innerHTML = chunks.length ? chunks.join("") : `<div class="empty">没有待提交的变更</div>`;

  els.log.innerHTML = status.log.length
    ? status.log.map((c) => `<li><span class="hash">${esc(c.hash)}</span><span class="subject">${esc(c.subject)}</span><span class="meta">${esc(c.author)} · ${esc(c.when)}</span></li>`).join("")
...
async function doCommit() {
  ...
  log(`开始 add + commit${els.push.checked ? " + push(" + target + ")" : ""}…`);
  try {
...
async function init() {
  ...
  els.sync.addEventListener("click", doSync);
  ...
  els.target.addEventListener("change", saveState);
